...
@interface JMBaseWebViewController ()<WKScriptMessageHandler,WKUIDelegate,WKNavigationDelegate>
...
-(void)setHTMLPath:(NSString *)path{
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:path relativeToURL:[[NSBundle mainBundle] bundleURL]]]];
...
- (WKWebView *)webView{
    ...
    if(_webView == nil){
        ...
        //自定义的WKScriptMessageHandler 是为了解决内存不释放的问题
         _weakScriptMessageDelegate = [[JMWeakWebViewScriptMessageDelegate alloc] initWithDelegate:self];
        //这个类主要用来做native与JavaScript的交互管理
        _wkUController = [[WKUserContentController alloc] init];
        //注册一个name为jsToOcNoPrams的js方法 设置处理接收JS方法的对象
        [_wkUController addScriptMessageHandler:_weakScriptMessageDelegate  name:@"jsToOcNoPrams"];
        [_wkUController addScriptMessageHandler:_weakScriptMessageDelegate  name:@"aaa"];
...
//
        NSString *jSString2 = @" $('meta[name=description]').remove(); $('head').append( '<meta name=\"viewport\" content=\"width=device-width, initial-scale=1,user-scalable=no\">' );";
        NSMutableString *javascript = [NSMutableString string];
        [javascript appendString:@"document.documentElement.style.webkitUserSelect='none';"];//禁止选择
        [javascript appendString:@"document.documentElement.style.webkitTouchCallout='none';"];//禁止长按
//        WKUserScript *script1 = [[WKUserScript alloc] initWithSource:jSString1 injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:NO];
        WKUserScript *script2 = [[WKUserScript alloc] initWithSource:jSString2 injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:NO];
        WKUserScript *noneSelectScript = [[WKUserScript alloc] initWithSource:javascript injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];

   
        //用于进行JavaScript注入

//        [wkUController addUserScript:script1];
        [_wkUController addUserScript:script2];
        [_wkUController addUserScript:noneSelectScript];
        [_wkUController addScriptMessageHandler:self name:@"openInfo"];
        config.userContentController = _wkUController;
